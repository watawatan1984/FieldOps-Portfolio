...
$requiredPatterns = [ordered]@{
    ...
    "public demo URL" = "公開デモ:\s*\[https://(fieldops\.by0\.uk/demo-login|fieldops-portfolio\.onrender\.com)\]"
    ...
    "verified live demo URL" = "Live demo:\s*\[(fieldops\.by0\.uk/demo-login|fieldops-portfolio\.onrender\.com)\]"
    ...
    "current verification date" = "2026-08-30、Windows / \.NET 10 / PostgreSQL 17 / Chromiumでローカル検証を再実行しました"
    "current test counts" = "Domain tests\s*\|\s*89/89[\s\S]*Integration tests\s*\|\s*227/227[\s\S]*Playwright E2E\s*\|\s*27/27[\s\S]*Full solution\s*\|\s*343/344"
    "load evidence" = "docs/evidence/load-test-results\.md"
    "free-host limitation" = "コールドスタート|scale-to-zero"
    "GAS public monitor" = "## GASによる公開監視"
    "business-hours ten-minute monitoring" = "10:00以上18:00未満に約10分おき.*health/live.*health/ready|health/live.*health/ready.*10:00以上18:00未満に約10分おき"
    "no monitoring outside window" = "時間外はFieldOpsへアクセスしません"
    "monitor limitation" = "10分監視が動く10:00〜18:00の間は通常スリープが抑えられます[\s\S]*保証付きで防ぐ機能ではありません|監視時間中は通常スリープが抑えられます[\s\S]*保証付きで防ぐ機能ではありません"
