...
internal sealed class SalesOpportunityConfiguration : EntityConfiguration<SalesOpportunity>
{
    protected override void ConfigureEntity(EntityTypeBuilder<SalesOpportunity> builder)
    {
        ...
        builder.Property(opportunity => opportunity.OwnerUserId).HasMaxLength(450);
        ...
        builder.HasIndex(opportunity => opportunity.OwnerUserId);
        ...
        builder.HasOne<ApplicationUser>().WithMany().HasForeignKey(opportunity => opportunity.OwnerUserId).OnDelete(DeleteBehavior.Restrict);
    }
}